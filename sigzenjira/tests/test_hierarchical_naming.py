...
class TestHierarchicalNaming(IntegrationTestCase):
	def test_name_spells_out_the_lineage(self):
		...
		task = make_task("NM Task", "Task", story.name, expected_time=4)
	...
	def test_counter_is_per_parent(self):
		...
		self.assertEqual(make_task("NM2 A-T1", "Task", story_a.name, 2).name, f"{story_a.name}-T-001")
		self.assertEqual(make_task("NM2 A-T2", "Task", story_a.name, 2).name, f"{story_a.name}-T-002")
		self.assertEqual(make_task("NM2 B-T1", "Task", story_b.name, 2).name, f"{story_b.name}-T-001")
	...
	def test_standalone_story_falls_back_to_a_global_counter(self):
		...
		self.assertEqual(make_task("NM3 T", "Task", story.name, 2).name, f"{story.name}-T-001")

	def test_reparenting_does_not_rename(self):
		epic = make_task("NM4 Epic", "Epic", expected_time=40)
		story_a = make_task("NM4 Story A", "Story", epic.name, expected_time=20)
		story_b = make_task("NM4 Story B", "Story", epic.name, expected_time=20)
		task = make_task("NM4 Task", "Task", story_a.name, 2)

		task.parent_task = story_b.name
		task.save()

		# name is a birth certificate, not a live path
		self.assertEqual(task.name, f"{story_a.name}-T-001")
	...
	def test_name_fits_the_column(self):
		...
		task = make_task("NM5 Task", "Task", story.name, 4)
